...
export default function balls() {
  ...
  // 点转化为Three.js的Euler
  const point2ThreeEuler = (point) =>
    new THREE.Euler(point.x, point.y, point.z);
  ...
  class Base {
    ...
    perspectiveCameraParams: any;
    orthographicCameraParams: any;
  ...
  const start = () => {
    const ballsElement = document.querySelector(".intro__balls");
    if (!ballsElement) return;
    const bouncyBalloon = new BouncyBalloon(".intro__balls", false);
    bouncyBalloon.init();
  };
